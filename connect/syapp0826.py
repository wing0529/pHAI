...
app.config['MAX_FILES'] = 10  # 업로드 가능한 최대 파일 수 설정
...
def wait_for_files(folder, num_files):
    while True:
        ...
        # Check if the required number of files have been uploaded
        if current_count == num_files:
            print(f"필요한 파일 수만큼 업로드했습니다: {current_count} 파일")
            return sorted(files)

        print(f"현재 업로드된 파일 수: {current_count} / 필요 파일 수: {num_files}")
        time.sleep(10)  # 5초 대기 후 다시 확인

@app.route('/', methods=['GET'])
def index():
    # result.json이 있으면 미리 지우기
    ...
    # JSON 응답 반환
    return jsonify({"message": "Files are being processed. Please check back later."})
...
def upload_multiple():
    ...
    if len(files) > app.config['MAX_FILES']:
        return jsonify({'error': f'You can upload a maximum of {app.config["MAX_FILES"]} files. Now = {len(files)}'}), 400
    ...
    # 업로드가 완료되면 JSON 응답으로 업로드된 파일 정보를 반환
...
def result():
    ...
    if os.path.exists(json_path):
        try:
            ...
            logging.error('Result JSON 파일을 찾을 수 없습니다.')
            return jsonify({'error': 'Result JSON 파일을 찾을 수 없습니다.'}), 404
    else:
        logging.error('result.json이 아직 생성되지 않음')
        return jsonify({'error': 'Result JSON 파일이 아직 생성되지 않았습니다.'}), 404
